...
registration.addEventListener('click', async (event) => {
        ...
        if (refs.password.value === '' || refs.login.value === '' || refs.age.value === ''|| refs.phoneNumber.value === '' || refs.name.value === '' || refs.gender.value === '' ) {
            PNotify.error('Fill in all the fields')
        } else {



  const registrationPost = await axios.post("https://venify.herokuapp.com/user/register", {
